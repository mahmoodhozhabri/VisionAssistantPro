...
- **Proveedor:** Selecciona tu servicio de IA preferido. Los proveedores compatibles incluyen **Google Gemini**, **OpenAI**, **Mistral**, **Groq** y **Personalizado** (servidores compatibles con OpenAI como Ollama, LM Studio, Jan.ai o KoboldCPP).
...
### 1.3.1 Configurar IA local (Configuración en un solo paso)
Para que la integración local de IA completamente sin conexión sea muy sencilla, hay un botón dedicado **"Configurar IA local"** disponible dentro de la Configuración del proveedor personalizado.

Si estás ejecutando un servidor de modelos de IA local en tu equipo:
1. Selecciona **Personalizado** como proveedor.
2. Presiona el botón **Configurar IA local**.
3. Elige tu motor de IA local en el diálogo accesible:
   - **Ollama** (por defecto `http://127.0.0.1:11434`)
   - **LM Studio** (por defecto `http://127.0.0.1:1234`)
   - **Jan.ai** (por defecto `http://127.0.0.1:1337`)
   - **KoboldCPP** (por defecto `http://127.0.0.1:5001`)
4. El complemento configurará al instante la URL local correcta, el tipo de API y obtendrá automáticamente tus modelos sin conexión activos para rellenar el selector de **Modelo de IA**.

*Nota sobre red y proxies:* Este motor de conexión local cuenta con un mecanismo avanzado de omisión de proxy. Aunque tengas activa una VPN del sistema o un proxy en modo TUN, las solicitudes a tu IA local lo omitirán completamente, garantizando conexiones sin conexión estables sin errores 502 Bad Gateway.
...
|---------------|-------------------------------|----------------------------------------------------------------------------------------------|
| **Shift + A** | **Operador IA**               | **Operación autónoma:** Dile a la IA que realice una tarea en tu pantalla. Presionarlo de nuevo detiene inmediatamente las operaciones activas. |
...
| **Shift + T** | Traductor de portapapeles     | Traduce el contenido actualmente en el portapapeles.                                         |
...
| **Shift + L** | **Gestionar/escanear etiquetas** | Abre el Gestor de etiquetas (si existen) o escanea la app en busca de elementos sin nombre. |
...
---
## Cambios para la versión 6.1.0

*   **Integración universal de IA local (Configurar IA local)**: Se añadió un nuevo botón **"Configurar IA local"** en la Configuración del proveedor personalizado. Los usuarios ahora pueden configurar automáticamente motores de IA local, incluyendo **Ollama**, **LM Studio**, **Jan.ai** y **KoboldCPP** de forma instantánea.
*   **Omisión inteligente de proxy local**: Se reconstruyó la lógica de conexión con un mecanismo avanzado de omisión de proxy. El complemento ahora es lo suficientemente inteligente para omitir completamente los proxies del sistema Windows para conexiones de bucle local, garantizando conexiones estables con la IA local incluso cuando tu VPN/modo TUN está activo.
*   **Etiquetado con IA ultraestable (v2)**: Se reemplazaron las coordenadas absolutas de pantalla por un sistema híbrido avanzado de **firma de objeto**. Las etiquetas ahora se basan en identificadores programáticos (UIA **AutomationId** o Win32 **ControlID**) y coordenadas relativas a la ventana, haciendo que tus etiquetas personalizadas sean completamente resistentes al redimensionamiento, movimiento, cambio de monitor o escalado de ventanas.
*   **Migración automática de etiquetas sin interrupciones**: La actualización es completamente transparente. El complemento migrará automáticamente tus etiquetas antiguas basadas en coordenadas al nuevo formato de huella digital estable en segundo plano al primer enfoque, sin pérdida de datos.

## Cambios para la versión 6.0

*   **Etiquetado semántico con IA**: Los usuarios ahora pueden etiquetar de forma permanente botones e iconos sin nombre usando IA. Presiona **L** para etiquetar el objeto navegador actual (compatible con foco por Tab y navegación por objetos) o **Shift+L** para escanear y etiquetar toda la aplicación de una vez.
*   **Gestor de etiquetas inteligente**: Se añadió un nuevo diálogo de Gestor de etiquetas completamente accesible (disponible con **Shift+L** si ya existen etiquetas) para ver, renombrar o eliminar etiquetas personalizadas en bloque.
...
*   **Operador IA (Control autónomo - Shift+A)**: Esta es la joya de la corona de la v5.5. Vision Assistant Pro ha pasado de ser un asistente pasivo a convertirse en tu **Operador IA** personal. Ya no solo describe la pantalla, sino que toma el control.
    *   *Cómo funciona:* Ahora puedes dar instrucciones verbales para operar tu PC. Por ejemplo, en una aplicación completamente inaccesible donde tu lector de pantalla permanece en silencio, puedes presionar **Shift+A** y escribir: *"Haz clic en el botón Configuración"* o *"Encuentra el campo de búsqueda, escribe 'Últimas noticias' y presiona Enter."* La IA identifica visualmente los elementos, mueve el ratón y ejecuta la tarea por ti.
...
\*   \*\*Capa de comandos:\*\* Se introdujo un sistema de Capa de comandos (predeterminado: `NVDA+Shift+V`) para agrupar atajos bajo una sola tecla maestra. Por ejemplo, en lugar de presionar `NVDA+Control+Shift+T` para traducción, ahora presionas `NVDA+Shift+V` seguido de `T`.
...
* Se añadió el comando nativo de OCR de archivo (NVDA+Control+Shift+F).
...
* Se estandarizaron todos los atajos para usar NVDA+Control+Shift para eliminar conflictos con el diseño de portátil de NVDA y las teclas de acceso rápido del sistema.
...
* Se añadió el comando dedicado de traducción desde portapapeles (NVDA+Control+Shift+Y).
